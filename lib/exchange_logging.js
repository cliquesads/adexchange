 ...
 * @param winning_bids winning bid object
 * @param bid_request openRTB bid request
 */
ExchangeCLogger.prototype.auction = function(err, placement, request, response, winning_bids, bid_request){
    var self = this;
    ...
    if (request.headers['user-agent'] != CLIQUESBOT_UA_STRING){
        bid_request.imp.forEach(function(imp){
            var auction_meta = {
                ...
                keywords: bid_request.site.keywords || null
            };
            if (err){
                if (err.constructor === {}.constructor){
                    err = JSON.stringify(err)
                }
                self.error("AUCTION ERROR raised during auction: " + err, auction_meta)
            }
            else if (winning_bids){
                var winning_bid = winning_bids[imp.id];
                var new_meta = {
                ...
                self.info("AUCTION", auction_meta);
            }
        });
    }
