...
var getDebugMarkup = markupGenerator.getDebugMarkup;
var pug = require('pug');
...
var getMultiPaneNativeTestMarkup = function(placement, pubURL){
    ...
    nativePlacementData.test = true;
    nativePlacementData.creativeSpecs = new Array(nativePlacementData.count).fill(config.get('Native.testCreative'));
    return nativePlacementData;
};
...
DefaultConditionHandler.prototype.main = function(bid_request, auction_error, placement, secure, parent_tag_type, pubURL, callback){
    ...
    secure = secure || false;
    var debugMarkup = getDebugMarkup(pubURL,null,bid_request, placement);
    // if native, return test creative markup if placement is inactive.
    // placement could be active & a native formFactor inactive, so only
    // serve test markup if placement is truly inactive at the placement level.
    // TODO: Fix w/ more obvious "staging" vs. "active" vs. "inactive" states
    if ((placement.type === 'native' || placement.type === 'multiPaneNative')
        && auction_error === getInactiveError(placement) && !placement.active){
        ...
        // add debug markup if applicable
        if (debugMarkup) nativeTestMarkup.debug = debugMarkup;
        // set default to false, since technically this is serving a test ad w/ test creative markup;
        nativeTestMarkup.default = false;
        ...
        try {
            switch (placement.defaultType){
                ...
                case 'hostedCreative':
                    ...
                    break;
                case 'hide':
                    // Only pass JSON object if native or multiPane native placement
                    // display ads don't get served JSON, they just get an iFrame as a string, so don't
...
DefaultConditionHandler.prototype.renderPassbackTag = function(passbackTag, parent_tag_type){
    ...
    if (parent_tag_type === 'iframe'){
        var fn = pug.compileFile('./templates/iframe_passback.pug', null);
