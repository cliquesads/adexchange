 ...
 * @private
 */
Auctioneer.prototype._create_bid_request = function(placement, req, cb){
    ...
    auctioneer.getBids(placement, request, pubURL, function (e, result, bid_request) {
        ...
        for (var impid in flattened_bids){
            if (flattened_bids.hasOwnProperty(impid)){
                ...
                var bids = flattened_bids[impid];
                auctions[impid] = function(callerbacker){
        ...
        // For multi-impression auctions, for now we will be defaulting if not all auctions
        // clear, so the upshot of using async.parallel is that it basically does this by default,
        // i.e. it will return err if ANY of the tasks error out.
