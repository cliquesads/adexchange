 ...
 *
 */
var getAuctionData = exports.getAuctionDataMacros = function(pubUrl, rawImpUrl, winningBidObject, placement){
    const fields = pubUrl.auctionDataFields.split(",");
    ...
    for (let f of fields) {
        switch (f) {
            ...
            case 'aid':
                ...
                break;
            case 'clearprice':
                ret['clearprice'] = crypto.encodeFloatWithIntegerKey(winningBidObject['clearprice'],URL_PARAM_CIPHER_KEY,2,'-',true);
                break;
            case 'price':
                ret['price'] = crypto.encodeFloatWithIntegerKey(winningBidObject['price'],URL_PARAM_CIPHER_KEY,2,'-',true);
                break;
            case 'pubid':
                ret[f] = placement.parent_publisher.id;
                break;
            case 'siteid':
                ret[f] = placement.parent_site.id;
                break;
            case 'pageid':
                ret[f] = placement.parent_page.id;
                break;
            default:
                // don't do anything, as this field value is not supported
                break;
        }
    }
    return ret;
...
var getMarkup = exports.getMarkup = function(req, placement, secure, winningBidObject, bid_request, pubURL){
    ...
    switch (placement.type){
        case "native":
            ...
            // now augment response markup with auctionData fields, if any were requested
            // If the client wants auctionData it must pass a CSV string of field names to
            // the 'auctionDataFields' param on the pubURL.
            if (pubURL.hasOwnProperty('auctionDataFields')){
                markup.exchangeAuctionData = getAuctionData(pubURL,adserverUrl, winningBidObject, placement);
            }
        ...
        case "multiPaneNative":
            ...
            for (var i = 0; i < sorted.length; i++){
                ...
                if (winningBidObject.hasOwnProperty(id)){
                    ...
                    // the 'auctionDataFields' param on the pubURL.
                    if (pubURL.hasOwnProperty('auctionDataFields')){
                        auctionData = getAuctionData(pubURL,adserverUrl, winningBidObject[id], placement);
