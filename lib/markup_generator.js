...
var getAuctionData = exports.getAuctionDataMacros = function(rawImpUrl, winningBidObject, placement){
    ...
    const ret = {
        ...
        'impid': impUrl.impid, 'aid': impUrl.aid,

        'pubid': placement.parent_publisher.id,
        'siteid': placement.parent_site.id,
        'pageid': placement.parent_page.id,
    };
    if (URL_PARAM_CIPHER_KEY){
        if (winningBidObject['clearprice']){
            try {
                const num = Number(winningBidObject['clearprice']);
                ret['clearprice'] = crypto.encodeFloatWithIntegerKey(num, URL_PARAM_CIPHER_KEY, 2, '-', true);
            } catch (e){
                console.error(`ERROR encoding clearprice in auctionData: ${e}`);
            }
        }
        if (winningBidObject['price']){
            try {
                const n = Number(winningBidObject['clearprice']);
                ret['price'] = crypto.encodeFloatWithIntegerKey(n, URL_PARAM_CIPHER_KEY, 2, '-', true);
            } catch (e){
                console.error(`ERROR encoding price in auctionData: ${e}`);
            }
        }
    }
    return ret
