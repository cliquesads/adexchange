 ...
 *
 */
var getAuctionData = exports.getAuctionDataMacros = function(pubUrl, rawImpUrl, winningBidObject){
    ...
    for (let f of fields) {
        switch (f) {
            ...
            case 'impid':
                ...
                break;
            case 'clearprice':
                if (winningBidObject.hasOwnProperty(f)) {
                    ret[f] = crypto.encodeFloatWithIntegerKey(winningBidObject[f],URL_PARAM_CIPHER_KEY,2,'-',true);
                }
                break;
            case 'price':
                if (winningBidObject.hasOwnProperty(f)) {
                ...
                break;
            default:
                // TODO: Encode encode-able values (i.e. price)
                if (winningBidObject.hasOwnProperty(f)) {
                    ret[f] = winningBidObject[f];
                }
                break;
        }
    }
    ...
    switch (placement.type){
        case "native":
            ...
            if (pubURL.hasOwnProperty('auctionDataFields')){
                markup.auctionData = getAuctionData(pubURL,adserverUrl, winningBidObject);
            }
        ...
        case "multiPaneNative":
            ...
            for (var i = 0; i < sorted.length; i++){
                ...
                if (winningBidObject.hasOwnProperty(id)){
                    ...
                    if (pubURL.hasOwnProperty('auctionDataFields')){
                        auctionData = getAuctionData(pubURL,adserverUrl, winningBidObject[id]);
                        markup.auctionData.push(auctionData);
                    }
                }
            }
            debugMarkup = getDebugMarkup(pubURL,winningBidObject,bid_request, placement);
