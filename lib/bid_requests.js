...
Auctioneer.prototype.get_bids = function(req, cb){
    ...
    this._create_single_imp_bid_request(req, function(err, request_data) {
        ...
        var bidder_timeout = this.bidder_timeout;
                ...
                request.post(post_options, function (error, response, body) {
                    ...
                    if (error) {
                        ...
                        if (error.code == 'ETIMEDOUT' || error.code == 'ESOCKETTIMEDOUT') {
                            this.logger.error('Bidder Timeout for bidder at URL %s', item, request_data)
                        } else {
                        ...
                        this.logger.error('HTTP Error Returned from bidder, Status Code ' + response.statusCode + ': ' + body);
                        if (body.constructor === {}.constructor){
                            this.logger.error(JSON.stringify(body, null, 2));
                        }
                        //return callback(err);
                    } else if (response.statusCode == 204) {
                        this.logger.info('HTTP 204 NOBID received');
                    } else {
