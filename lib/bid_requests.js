...
function run_auction(bid_objects){
    //sort bids objects in descending order
    //synchronous as far as I can tell, not including callback
    var bids_sorted = bid_objects.sort(function(a, b){
        return b.bid - a.bid;
    });
    var winning_bid_obj = bids_sorted[0];
    winning_bid_obj['clear_price'] = bids_sorted[1].bid + 0.01;
...
get_bids(bid_urls, function(err, result){
    if (err) throw err;
    winning_bid = run_auction(result);
