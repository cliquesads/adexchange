...
function _parse_geo_data(req, callback){
    /* Parses geo-data based on IP address.  Currently uses MaxMind for
     Geo-IP lookup. */
    //var tmp = '73.47.234.128';
    //var tmp = '72.229.28.185';
    //var location = maxmind.getLocation(tmp);
...
function _handle_bid_response(error,response,body,callback){
    ...
    if (error) {
        ...
        var err = "HTTP Error Returned from bidder, Status Code " + response.statusCode + ": " + body;
        return callback(err);
    } else {
...
function handle_default_condition(request, response){
    ...
    response.status(200).json({"adm": config.get(util.format('Exchange.defaultcondition.%s', dim))});
}