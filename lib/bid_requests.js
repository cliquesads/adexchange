...
function _parse_imp_and_site_data(req, callback){
    ...
    db.getPublisherData(req, function(err, pub_data) {
    ...
    try {
        async.parallel(
            [
                ...
                function (callback) {
                    _parse_imp_and_site_data(req, function(err, imp_and_site_data){
                        if (err) throw err;
...
function get_bids(bid_urls, req, logger, cb){
    ...
    _create_single_imp_bid_request(req, function(err, request_data) {
        ...
        try {
            async.parallel(a_tasks, function () {
...
function handle_default_condition(request, response){
    ...
    response.status(200).json({"adm": config.get('Exchange.defaultcondition.300x250'), "default": true});
}