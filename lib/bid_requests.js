 ...
 *
 * Whole sequence can be conveniently executed through Auctioneer.main method@bidders bidder object must have "url","nurl" and "timeout" properties
 * @class Auctioneer
 ...
 * @param mongo_connection mongoose connection to MongoDB
 * @param logger logger instance which inheritst from winston.logger
 * @constructor
 */
var Auctioneer = function(bidders, bidder_timeout, mongo_connection, logger){
    ...
    this.connection = mongo_connection;
    this.logger = logger;
 ...
 * @private
 */
Auctioneer.prototype._parse_imp_and_site_data = function(req, callback){
    /* */

    //make DB call first, populate data in callback
    //var pub_data = {imp_data: {}, site_data: {}};
    db.getPublisherData(req, this.connection, function(err, pub_data) {
 ...
 * @private
 */
Auctioneer.prototype._create_single_imp_bid_request = function(req, cb){
    ...
    async.parallel(
        [
            ...
            function (callback) {
                auctioneer._parse_imp_and_site_data(req, function(err, imp_and_site_data){
                    if (err) return callback(err);
 ...
 */
Auctioneer.prototype.getBids = function(req, cb){
    ...
    this._create_single_imp_bid_request(req, function(err, request_data) {
                ...
                request.post(post_options, function (error, response, body) {
                    ...
                    if (error) {
                        ...
                        if (error.code == 'ETIMEDOUT' || error.code == 'ESOCKETTIMEDOUT') {
                            auctioneer.logger.error('Bidder Timeout for bidder at URL %s', item, request_data)
                        } else {
                            //just log any other types of errors
                            auctioneer.logger.error(error);
                        }
                    } else if (response.statusCode != 200 && response.statusCode != 204) {
                        // catches any HTTP error (Non 200 or 204) thrown by bidder
                        auctioneer.logger.error('HTTP Error Returned from bidder, Status Code ' + response.statusCode + ': ' + body);
                        if (body.constructor === {}.constructor){
                            auctioneer.logger.error(JSON.stringify(body, null, 2));
                        }
                        //return callback(err);
                    } else if (response.statusCode == 204) {
                        auctioneer.logger.info('HTTP 204 NOBID received');
                    } else {
 ...
 */
Auctioneer.prototype.main = function(request, response, callback){
    var auctioneer = this;
    var logger = this.logger;
    //first get all bids from bidders
    auctioneer.getBids(request, function (e, result) {
        ...
        auctioneer.runAuction(result, function (er, winning_bid) {
            ...
            //if successful, send winning markup in respone to pub tag
            ...
            // currently calling callback before win response is sent, don't really know
            // what to do with it as it's not necessary as currently implemented, but could be
